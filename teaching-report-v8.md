# 🦞 OpenClaw 全方位教學報告 V8.0

> **報告版本**：V8.0  
> **適用版本**：OpenClaw v2026.4.x 教學期  
> **用途**：段落 0–9 每區塊拆分為 1–8 頁（本版補強 OpenShell / NemoClaw / OpenClaw sandbox backend 關係）  | **總頁數**：47 頁
...
> **V8 補充重點**：OpenShell 不只是 NemoClaw 的概念。**OpenClaw core 已可直接把 sandbox backend 設成 `openshell`**；NemoClaw 則是在此基礎上，再額外加上 guided onboarding、hardened blueprint、managed inference、policy 與 NVIDIA 生態整合。
...
> **V8 校正**：前一版容易讓人誤以為 OpenClaw 只有 Docker / SSH / Native。根據官方 `docs/gateway/openshell.md`、`docs/cli/sandbox.md` 與 `extensions/openshell/`，**OpenClaw core 現在已經原生支援 OpenShell sandbox backend**。
