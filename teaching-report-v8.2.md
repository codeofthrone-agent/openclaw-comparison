# 🦞 OpenClaw 全方位教學報告 V8.2

> **報告版本**：V8.2  
> **適用版本**：OpenClaw v2026.4.x 教學期  
> **用途**：段落 0–9 每區塊拆分為 1–8 頁（本版校正 OpenShell / NemoClaw / OpenClaw sandbox backend 關係，並補上 IronClaw 高安全分但多 X 的評分邏輯）  | **總頁數**：47 頁
...
│  第 6 站 📊  14 變體對比：選對你的 Claw                      │
│  第 7 站 🚀  部署上線與未來展望                             │
│  第 8 站 🧭  教學導航/速選/避坑/成本/實戰補全                │
│  第 9 站 ☁️  線上部署評估：OpenAB 與 Claude 雲端方案         │
│                                                            │
│  🎯 我們的目標：                                             │
...
347K ┤                                   ★ OpenClaw (TS, 347K)
 ...
 14K ┤★ CoPaw (Python, 14K)               ★ IronClaw (Rust, 11K)      ★ NemoClaw (JS/TS, 18.8K)
  7K ┤                                    ★ NullClaw (Zig, 7K)        ★ LobsterAI (TS, 5K)
...
### ☁️ 方法四：OpenShell Sandbox Backend（企業 / 受管沙盒）

> **V8.1 補充重點**：OpenShell 不只是 NemoClaw 的概念。**OpenClaw core 已可直接把 sandbox backend 設成 `openshell`**；NemoClaw 則是在此基礎上，再額外加上 guided onboarding、hardened blueprint、managed inference、policy 與 NVIDIA 生態整合。
...
```bash
# 驗證 OpenShell sandbox runtime
openclaw sandbox list
openclaw sandbox explain
```

**理解方式：**
- **OpenClaw** = agent framework / gateway / tools / memory / channels
- **OpenShell** = 受管沙盒 runtime / remote workspace / SSH-based exec
- **NemoClaw** = 把 OpenClaw 放進 OpenShell，並提供更 opinionated 的安全與 onboarding 包裝

### 2.5 OpenClaw / OpenShell / NemoClaw 三層關係圖

```text
┌──────────────────────────────────────────────────────────────┐
│ Layer 3: NemoClaw                                            │
│ NVIDIA reference stack / onboarding / hardened blueprint /   │
│ managed inference / opinionated policy                       │
└───────────────────────┬──────────────────────────────────────┘
                        │ packages + orchestrates
                        ▼
┌──────────────────────────────────────────────────────────────┐
│ Layer 2: OpenShell                                           │
│ managed sandbox runtime / remote workspace / SSH exec /      │
│ policy-aware environment lifecycle                           │
└───────────────────────┬──────────────────────────────────────┘
                        │ runs backend for
                        ▼
┌──────────────────────────────────────────────────────────────┐
│ Layer 1: OpenClaw                                            │
│ agent framework / gateway / channels / tools / memory /      │
│ skills / workflows / multi-agent runtime                     │
└──────────────────────────────────────────────────────────────┘
```

**一句話解釋：**
- **OpenClaw** 負責「Agent 做什麼」
- **OpenShell** 負責「Agent 在哪裡被安全地執行」
- **NemoClaw** 負責「把 OpenClaw + OpenShell 打包成更可落地的 NVIDIA 安全部署路線」
...
[2] Tool Policy    →  tools.allow / tools.deny + tools.profile + tools.byProvider
   │ 通過
   ▼
[3] Sandbox        →  Docker (seccomp/AppArmor) / SSH / OpenShell / Native
   │ 通過
   ▼
...
| **SSH 遠端伺服器** | ★★★★☆ | 物理隔離 |
| **OpenShell 受管沙盒** | ★★★★☆ | 由 `openshell` CLI 建立 remote sandbox，透過 SSH-based command execution + 可選 mirror/remote workspace 模式 |
| **原生 OS 隔離** | ★★★☆☆ | Workspace 目錄限制 |

```yaml
sandbox:
  mode: "all"
  backend: "openshell"      # docker / ssh / openshell / native
plugins:
  entries:
    openshell:
      enabled: true
      config:
        mode: "remote"      # remote / mirror
        from: "openclaw"

# 若使用 docker backend，仍可搭配 seccomp / AppArmor / network isolation
docker:
...
> **V8.1 校正**：前一版容易讓人誤以為 OpenClaw 只有 Docker / SSH / Native。根據官方 `docs/gateway/openshell.md`、`docs/cli/sandbox.md` 與 `extensions/openshell/`，**OpenClaw core 現在已經原生支援 OpenShell sandbox backend**。
...
- 在安全防護中，它用來**驗證設定檔與工具輸出格式**，避免錯誤或惡意輸入。
- 好處：**把「不符合規格的輸入」擋在系統外**，提高整體安全性與可靠性。

> ✅ 心法：**先定義規格（Zod）→ 再允許輸入 → 再執行。**
...
> **V8.2 校正重點**：這張表不能只看 ✅/❌ 的數量。**IronClaw 安全評分高，不是因為它功能覆蓋最廣，而是因為它把安全重心壓在更底層的「安全原語」**：Rust 記憶體安全、WASM 隔離、AES-256-GCM、Prompt Injection 防禦、OAuth。它缺少很多 OpenClaw 式的治理層（DM Policy、Tool Policy、Audit、Zod），所以表上會有很多 X；但底層隔離與加密仍然非常強。

| 安全能力 | OpenClaw | Hermes | NanoClaw | IronClaw | ZeroClaw |
|:---|:---:|:---:|:---:|:---:|:---:|
| **執行隔離模型** | ✅ Docker / SSH / OpenShell | ✅ 6 種 sandbox backend | ✅ 預設容器 | ✅ WASM | ✅ Landlock |
| **seccomp/AppArmor** | ✅ | ❌ | ✅ | ❌ | ❌ |
| **DM Policy** | ✅ | ✅ | ❌ | ❌ | ❌ |
| **Tool Policy** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **Tool Result Guard** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **Zod Validation / Schema 驗證** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **Security Audit** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **Credential / Secret 保護** | ✅ SecretRef | 憑證池 | ❌ | ✅ AES-256-GCM | ❌ |
| **Prompt Injection 專項防禦** | ⚠️ 間接（Tool Policy / SSRF / Guard） | ❌ | ❌ | ✅ | ❌ |
| **危險命令 Regex** | ❌ | ✅ | ❌ | ❌ | ❌ |
| **審批機制** | ❌ | ✅ | ❌ | ❌ | ❌ |

#### 為什麼 IronClaw 分數最高，卻有很多 X？

```text
OpenClaw   = 治理型安全（Policy / Audit / Validation / Guard）
Hermes     = 互動型安全（Dangerous command regex / approval）
IronClaw   = 原語型安全（Rust / WASM / AES-256-GCM / Prompt defense）
```

- **OpenClaw**：安全能力覆蓋面最廣，像一個企業控制平面；很多格子都有 ✅。
- **Hermes**：重點在「執行前攔截」與「人類審批」，不是完整治理鏈。
- **IronClaw**：雖然缺少 DM Policy、Tool Policy、Audit、Zod 等治理機制，但它在**底層攻擊面收縮**做得最激進：
  - Rust 記憶體安全
  - WASM 沙箱（非傳統容器，但隔離非常強）
  - AES-256-GCM 憑證保護
  - Prompt Injection 專項防禦
  - NEAR AI OAuth

#### 評分邏輯（V8.2 明確化）

> **安全評分 ≠ 打勾數量。**
> 它衡量的是：**底層隔離強度 + 秘密保護 + 攻擊面收縮 + 治理/審計成熟度** 的綜合結果。

因此：
- **IronClaw 高分** = 因為底層原語非常強
- **OpenClaw 高覆蓋** = 因為治理鏈完整
- **表格很多 X** = 代表它**沒有採用 OpenClaw 那一套治理型控制面**，不代表它整體不安全

> **💡 關鍵洞察**：OpenClaw 贏在「治理層完整度」，Hermes 贏在「互動前審批」，IronClaw 贏在「底層安全原語」。三者不是同一種安全哲學，所以不能只靠 ✅ 數量排序。
...
| 5 | **NemoClaw** | 18.8K | JS/TS | OpenShell 參考堆疊（封裝 OpenClaw） | — | — | 企業/封閉環境 |
| 6 | **CoPaw** | 14K | Python | 中國雲部署 | 62/100 | 2/10 | 多平台 |
| 7 | **IronClaw** | 11K | Rust | 隱私安全 | **92/100** | 2/10 | 4 |
| 8 | **NullClaw** | 7K | Zig | 極速極小 | 50/100 | 2/10 | 19 |
| 9 | **LobsterAI** | 5K | TS | 全場景 Cowork | 68/100 | 2/10 | 全平台 |
| 10 | **TinyAGI** | 3K | TS | 個人公司 | 60/100 | 7.5/10 | 3 |
| 11 | **Moltis** | 2K | Rust | 可信任單一 | 85/100 | 2/10 | 內建 |
| 12 | **ZeptoClaw** | 556 | Rust | 實驗極輕量 | 45/100 | 1/10 | CLI |
| 13 | **RivonClaw** | 252 | TS | OpenClaw UI 層 | 70/100 | 7/10 | 繼承 25+ |
| 14 | **Hermes** | 開源 | Python | Provider-agnostic | 75/100 | 7/10 | ~17 |
| — | **參照** | — | — | 獨立框架 | — | — | — |

> **評分註解（V8.2）**：IronClaw 的高分主要來自 **Rust + WASM + AES-256-GCM + Prompt Injection 防禦** 這類底層安全原語，不代表它在 DM Policy / Tool Policy / Audit / Zod 這些治理層也最完整。
...
## 段落 8️⃣ 教學導航與避坑補全 — 第 1 頁

### 🚀 30 秒快速選擇：哪種部署方式適合你？

> 來源：awesome-openclaw-tutorial README（30秒快速选择）

| 你的情況 | 推薦方案 | 為什麼 | 開始時間 |
|:--|:--|:--|:--|
| 完全新手、想最快體驗 | 飛書妙搭 | 免費 + 1 分鐘完成 + 每日 100 萬 Tokens | 立即開始 |
| 有伺服器、想可視化管理 | 寶塔面板 | 免費插件 + 面板管理 + 一鍵安裝 | 3 分鐘搞定 |
| 企業級、高安全需求 | JVSClaw | 14 天免費 + 6核12GB + 端到端加密 | 需邀請碼 |
| 想用瀏覽器操控 | Kimi Claw | Kimi K2.5 + 瀏覽器控制 | 200 元/月 |
| 量化交易 / 多 IM | 腾讯 WorkBuddy | 桌面端 Agent + 多 IM 支援 | 送 5000 積分 |
| macOS 原生體驗 | QClaw | 腾讯官方桌面客戶端 | 免費 |
| Agent 生態豐富 | 扣子 OpenClaw | 1800+ Skills | 99 元/月起 |

**完整一鍵部署**：
https://github.com/xianyu110/awesome-openclaw-tutorial/blob/main/Openclaw史上最简单教程，小白一键部署.md

---

## 段落 8️⃣ 教學導航與避坑補全 — 第 2 頁

### 🧭 教程導航（按學習路徑）

> 來源：awesome-openclaw-tutorial README（教程导航）
...
完整導航：
https://github.com/xianyu110/awesome-openclaw-tutorial#-教程导航按学习路径

---

## 段落 8️⃣ 教學導航與避坑補全 — 第 3 頁

### 🆘 常見問題 & 快速解法

> 來源：awesome-openclaw-tutorial README（遇到问题？快速解决）

**安裝/配置**
- 安裝失敗 → 檢查 Node 版本與平台要求
- API 連線失敗 → 檢查 auth-profiles / API Key
- 飛書 Bot 不回 → 檢查多平台集成段落

**使用問題**
- AI 變「啞巴」→ 將 tools.profile 切到 `full`
- Gateway 啟動失敗 → 檢查 gateway.auth.mode

**成本優化**
- API 費用過高 → 改用國產模型（DeepSeek、Kimi）

完整 FAQ：
https://github.com/xianyu110/awesome-openclaw-tutorial/blob/main/appendix/E-common-problems.md

---

## 段落 8️⃣ 教學導航與避坑補全 — 第 4 頁

### 🚨 重要版本提示（Breaking Change）

> 來源：awesome-openclaw-tutorial README（重要版本提示）

**2026.3.7：Gateway 認證要求（Breaking Change）**
```bash
...
**2026.3.24：Node 版本要求提升**
- macOS ≥ 22.16
- Linux/Windows ≥ 22.14

---

## 段落 8️⃣ 教學導航與避坑補全 — 第 5 頁

### 💰 成本對比

> 來源：awesome-openclaw-tutorial README（成本对比）

| 方案 | 月費 | 適用場景 |
|:--|:--|:--|
| 飛書妙搭 | 免費 | 新手推薦（限時） |
| 雲端部署 | 20–50 元 | 無 Mac / 需 24 小時運行 |
| 本地部署 | 0 元 | 有 Mac 電腦 |
| API（DeepSeek） | 5–30 元 | 日常使用 |
| API（Kimi） | 10–50 元 | 長文檔處理 |

省錢提示：改用國產模型可省 50%–70%。

---

## 段落 8️⃣ 教學導航與避坑補全 — 第 6 頁

### 🎯 實戰案例精選（直接套用）

> 來源：awesome-openclaw-tutorial README（实战案例精选）

**配置範例**
- 基礎配置（basic-config）
- 多模型配置（multi-model）
- 多 Agent 配置（multi-agent）
- 飛書 Bot 配置（feishu）
...
參考連結：
https://github.com/xianyu110/awesome-openclaw-tutorial#-实战案例精选

---


## 段落 9️⃣ 線上部署評估：OpenAB 與 Claude 雲端方案 — 第 1 頁

### ☁️ 為什麼要單獨評估「線上部署」？

本章補的是 **v6 還沒有講清楚的東西**：
不是只有「OpenClaw 能不能裝起來」，而是 **哪一種雲上形態最適合長期運行、多人共用、低維護、低爆炸半徑**。
...
雲端橋接型
   ↓
Discord / GitHub / Slack → Broker / Action → LLM / CLI
   ↓
優點：24x7、集中控管、方便多人協作
缺點：權限、憑證、成本、隔離要設計好
```
...
## 段落 9️⃣ 線上部署評估：OpenAB 與 Claude 雲端方案 — 第 2 頁

### 🧩 OpenAB：雲端 Discord→ACP Broker 的定位

> 來源：openab README + GitHub Repo Stats
> - Repo：`openabdev/openab`
> - Stars：185（2026-04-09 查詢）
> - 定位：**lightweight / secure / cloud-native ACP harness**

OpenAB 不是 OpenClaw 那種「個人 AI OS」。
它更像是一個 **Discord 到 coding CLI 的雲端橋接器**：
...
**它強的地方**
- 很窄：只專注在 Discord + ACP-compatible coding CLI
- 很雲：README 直接提供 Dockerfile、K8s manifests、Helm chart
- 很像 broker：一個 thread 對一個 CLI session，適合 coding bot 場景
- 支援多 agent：Helm `agents` map 可一個 release 開多個 agent deployment

**它弱的地方**
- 不是完整 OpenClaw 替代品
- 主要面向 coding CLI，不是生活助理 / knowledge hub / 全平台 routing OS
- 入口目前核心是 Discord，不是多平台超級入口

**判斷**：
如果你要的是 **「雲上 coding agent broker」**，OpenAB 很對位；
如果你要的是 **「多平台、多技能、多工作流、個人助理框架」**，它不是 OpenClaw 的全功能替身。

---

## 段落 9️⃣ 線上部署評估：OpenAB 與 Claude 雲端方案 — 第 3 頁

### 🏗️ OpenAB 線上部署成熟度評估

OpenAB README 已直接給出兩條成熟路徑：
1. **單機/容器**：`cargo run`、release binary、Docker
2. **Kubernetes / Helm**：每個 agent key 生成 Deployment / ConfigMap / Secret / PVC

| 評估項 | OpenAB 表現 | 觀察 |
|:--|:--|:--|
| 雲端原生度 | 高 | README 直接寫明 Kubernetes-ready、Helm install |
| 多 Agent | 中高 | Helm 可同 release 跑 kiro + claude 等多 agent |
| 狀態持久化 | 中高 | PVC 保存 CLI auth / settings |
| 平台整合 | 低到中 | 核心偏 Discord，不像 OpenClaw 多平台廣 |
| 工作流能力 | 中 | 依賴下游 CLI / ACP，不是自己內建大量 workflow 層 |
| 維運複雜度 | 中 | 比完整 OpenClaw 輕，但仍要處理 bot token、CLI auth、PVC |

**最值得注意的風險**
- session pool = thread 對應 CLI process，**高併發時要管 max_sessions**
- CLI auth 依賴 PVC 持久化，token/state 壞掉會影響該 agent
- 其優勢來自「單一任務很專」；一旦想加知識庫/長期記憶/跨平台 orchestration，就會開始碰到邊界

**一句話**：
OpenAB 很適合做 **線上 Discord coding agent gateway**，
但不適合直接當成 OpenClaw 教學報告裡的「總平台終局」。

---

## 段落 9️⃣ 線上部署評估：OpenAB 與 Claude 雲端方案 — 第 4 頁

### 🤖 最新 Claude 雲部署：官方已走向「多雲 + CI/CD + 官方動作」

> 來源：Anthropic 官方 Claude Code Docs / GitHub repos
> - `anthropics/claude-code-action` Stars：6964（2026-04-09 查詢）
> - 官方支援：**Direct API / AWS Bedrock / Google Vertex AI / Microsoft Foundry**

目前「最新 Claude 雲部署」不是只有一種型態，而是至少 3 條主線：

#### A. GitHub Actions 上跑 Claude Code（最成熟）
- 官方 `claude-code-action@v1`
- 可在 PR / Issue / review flow 中用 `@claude`
- **運行在你自己的 GitHub runner 上**，不是 Anthropic 替你 host 你的 repo 執行環境
- 適合：CI review、修 bug、自動改 PR、issue automation

#### B. Claude Code + 雲模型供應商
- **Direct Anthropic API**：最直
- **AWS Bedrock**：官方 docs 已有獨立部署頁，並提醒多使用者情境要 **pin model versions** 避免新模型釋出造成 breakage
- **Google Vertex AI**：官方 docs 已有獨立部署頁，支援 **global / regional endpoints**；申請模型存取可能要 **24–48 小時**
- **Microsoft Foundry**：官方 action/docs 已列為正式支援路徑

#### C. Web / Hosted 入口
- `claude.ai/code` 屬於 Anthropic 自家託管入口
- 優點：最快開始
- 缺點：企業級控制、客製化路由、與你自己的 bot/gateway 編排程度較低

### 最終比較：OpenClaw vs OpenAB vs Claude 官方雲部署

| 方案 | 最強場景 | 優點 | 主要限制 |
|:--|:--|:--|:--|
| OpenClaw 線上部署 | 多平台個人/團隊 Agent OS | 功能最完整、擴展性高 | 維運最重 |
| OpenAB | Discord 雲端 coding broker | 架構輕、K8s/Helm 友善、ACP CLI 接得快 | 產品面較窄 |
| Claude Code Action / 官方雲路徑 | GitHub 原生開發流程 | 官方支持最強、CI/CD 最成熟、多雲可選 | 偏 repo / CI 場景，不是通用多平台助理框架 |

### 建議落地順序
1. **只想做 repo 自動化** → 先上 Claude Code Action
2. **只想做 Discord coding agent** → OpenAB 很划算
3. **想做真正多平台、多 agent、帶 workflow/skills 的完整系統** → 還是 OpenClaw 路線

**關鍵原則**：
雲端部署不是比「誰比較酷」，而是比 **誰的責任邊界最清楚、最不容易整組一起炸掉**。
